...
if(@ARGV < 7) {
    print STDERR "Usage: MACS_21ReadCount_BinarySearch_n6amt1.pl header_line(T/F, only \"T\" will print the header line) map_stat_file(for normalization) pos_file bed_dir outf1(count) outf2(count_norm) input_prefixs(eg. ISVA1PFC)\n";
    exit(0);
}
...
while(<IN>){
	if($_=~/^Sample/){
		...
		my @info=split(/\t/,$_);
		if($info[0]=~/s_\d_([^\s]+)/){
			$total_reads{$1}+=$info[4];
			$total_reads{$info[0]}+=$info[4];
		}else{
			### using properly paired mapped reads with mapping quality >=20; 
			$total_reads{$info[0]}+=$info[4];
		}
	}
}
...
#				my $cmd=qq(samtools view -c $bam $chr:$ss-$ee);
#				my $count=`$cmd`;
			my $count_norm;
			if($count==0){
				$count_norm=0;
...
#				my $t = threads->create(\&get_count, $bam, $chr, $pos, $ss, $ee);
		}
...
sub get_fragment_count{
	...
	while(1){
		...
		### test
#		print STDERR "Times:$times\tmid:$mid\tpos_start:$pos_start\n";
#		print STDERR "$lines[$mid]\n\n";
		### test
		if($times==8192){  ### 2**13
		...
		### test
#		print STDERR "Last\nTimes:$times\tmid:$mid\tpos_start:$pos_start\n";
#		print STDERR "$lines[$mid]\n\n";
		### test
...
#			print STDERR "Exit: $info[0]\t$info[1]\n";
			last;
		}
	}
	return $count;
